...
local function UpdateModeButton()
    ...
    elseif listMode == "graylist" then
        modeToggleButton:SetText("Graylist")
        modeToggleButton:GetFontString():SetTextColor(0.7, 0.7, 0.7)
    end
end
...
local function CreateEntry(name, reason)
   ...
   elseif listMode == "graylist" then
    entry.reason:SetTextColor(0.7, 0.7, 0.7)
end
...
function LoadList(searchTerm)
    ClearEntries()
    if not NuggetListDB then NuggetListDB = { blacklist = {}, whitelist = {}, graylist = {} } end
...
local function ShowTextWindow(title, initialText, onAccept)
    ...
    scrollFrame:SetScript("OnMouseWheel", function(self, delta)
        ...
        local step = 20
        self:SetVerticalScroll(math.min(max, math.max(0, current - delta * step)))
    end)
...
autoSellFrame:SetScript("OnEvent", AutoSellGrays)
		...
		local graylistIcon = "|TInterface\\AddOns\\NuggetList\\nuggeticon_gray.tga:16:16|t "
...
local function IsGraylisted(name)
    return NuggetListDB and NuggetListDB.graylist and NuggetListDB.graylist[name] ~= nil
end
...
local function AddNuggetIconToChat(self, event, msg, author, ...)
    ...
    elseif IsGraylisted(cleanName) then
        msg = graylistIcon .. msg
    end
